'use strict';
const { asn1, pki, util, pkcs7, options } = require('node-forge');
...
class Firmafiel {
   ...
   */
  verificarValidez(llavePublica, url = oscpCheckerUrl) {
    const cert = this.getCertificado(llavePublica);
  ...
  /*
   * Normaliza un certificado (llave pública) al formato requerido.
   */
  getCertificado(data) {
   ...
   */
  hasAtributoValor(llavePublica, valor) {
    const attributes = Object.values(this.getSubjectAttributes(llavePublica));
   ...
   */
  desencriptarLlavePrivada(llavePrivada, contrasena) {
    try {
   ...
   */
  validarLlaves(llavePublica, llavePrivadaDesencriptada) {
    const { publicKey } = this.getCertificado(llavePublica);
  ...
  firmarCadena({
    ...
    const decryptedKey = this.decryptPrivateKey(llavePrivada, contrasena);
    if (!this.validarLlaves(cert, decryptedKey)) {
      throw new Error(
